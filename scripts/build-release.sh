...
# Собранный так DMG приезжал бы пользователю без Parakeet v3 — а именно его
# онбординг скачивает и выставляет движком новому пользователю, так что тот
# упёрся бы в «Parakeet v3 недоступен в этой сборке» на первом же экране.
# `make app` остаётся быстрым whisper-only путём для разработки.
#
